...
func is_grounded():
	if not on_floor and $mid_ray.is_colliding():
		print("first contact")
		on_floor = true
	elif on_floor and not $mid_ray.is_colliding():
		print("lift off")
		on_floor = false
	return on_floor
...
func set_movement_dir():
	...
	direction -= transform.basis.z * Input.get_action_strength("move_ahead")
	direction += transform.basis.z * Input.get_action_strength("move_back")
